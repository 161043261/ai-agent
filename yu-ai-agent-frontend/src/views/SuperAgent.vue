...
const sendMessage = (message) => {
  ...
  // 处理结构化消息
  const handleStructuredMessage = (msg) => {
    switch (msg.type) {
      case "step_start":
        // 步骤开始 - 不显示步骤信息
        break;
      case "thinking":
        // AI 思考内容
        if (msg.content?.trim()) {
          createBubble(`💭 ${msg.content}`, "ai-thinking");
        }
        break;
      case "tool_call":
        // 工具调用
        createBubble(`🔧 调用工具: ${msg.tool}`, "ai-tool-call");
        break;
      case "tool_result":
        // 工具结果 (截取前200字符)
        const result =
          msg.result?.length > 200
            ...
            : msg.result;
        createBubble(`📋 ${msg.tool} 结果: ${result}`, "ai-tool-result");
        break;
      case "step_result":
        // 步骤结果 (可选显示)
        break;
      case "finished":
        createBubble(`✅ ${msg.content}`, "ai-final");
        break;
      case "error":
        createBubble(`❌ ${msg.content}`, "ai-error");
        break;
      default:
        // 未知类型, 显示原始内容
        if (msg.content) {
          createBubble(msg.content);
        }
    }
  };
